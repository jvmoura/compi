...
public class Codegen {
    ...
    if (s instanceof SEQ) {
        ...
        munchJUMP((JUMP)s); // Verifique se esta chamada está correta
    }
    else if (s instanceof EXP) {
        // Uma instrução EXP(CALL(...)) deve ser tratada como uma chamada.
    ...
    if (s.dst instanceof MEM && s.src instanceof MEM) {
        ...
        // O template "MOVE M[`s0] <- M[`s1]" usa dois registradores de origem.
        TempList srcList = new TempList(tempDst, new TempList(tempSrc, null));
    ...
    if (s.dst instanceof MEM && s.src instanceof Expr) {
        ...
        if (memDst.exp instanceof BINOP) {
            ...
            if (binop.binop == BINOP.PLUS) {
                if (binop.right instanceof CONST) {
                    Temp t1 = munchExp(binop.left); // Registrador base
                    Temp t2 = munchExp(s.src);       // Valor a ser armazenado
                    // ✅ CORRIGIDO: t1 e t2 estão na mesma lista de origem (src)
                    TempList srcList = new TempList(t1, new TempList(t2, null));
                    emit(new OPER("STORE M[`s0 +" + ((CONST) binop.right).value + "] <- `s1\n", null, srcList));
                    return;
                } else if (binop.left instanceof CONST) {
                    Temp t1 = munchExp(binop.right); // Registrador base
                    Temp t2 = munchExp(s.src);        // Valor
                    // ✅ CORRIGIDO: t1 e t2 estão na mesma lista de origem (src)
                    TempList srcList = new TempList(t1, new TempList(t2, null));
        ...
        TempList srcList = new TempList(munchExp(s.src), null);
        // Este template "STORE M[`s0] <- `s1" também precisa de uma lista de origem com 2 elementos
        emit(new OPER("STORE M[`s0] <- `s1\n", null, new TempList(dstList.head, srcList)));
        return;
    }
    
    if (s.dst instanceof TEMP) {
        Temp dstTemp = ((TEMP) s.dst).temp;
        Temp srcTemp = munchExp(s.src);
        emit(new Assem.MOVE("move `d0, `s0", dstTemp, srcTemp));
    }
}
    ...
    void munchJUMP(JUMP s) {
        Temp targetRegister = munchExp(s.exp);
        TempList srcList = new TempList(targetRegister, null);
        // CORRIGIDO: Fornece a lista de origem (srcList) para o placeholder 's0'.
        emit(new OPER("JUMP `s0\n", null, srcList, s.targets));
    }
    ...
    Temp munchNAME(NAME e) {
    Temp r = new Temp();
    // Usa a instrução 'la' (load address) do MIPS
    emit(new OPER("la `d0, " + e.label.toString(), new TempList(r, null), null));
    return r;
}

// E modifique munchExp para incluir o novo caso
Temp munchExp(Expr e) {
    ...
    if (e instanceof TEMP)   return munchTEMP((TEMP)e);
    ...
    if (e instanceof NAME) {
        Temp r = new Temp();
        emit(new OPER("la `d0, " + ((NAME)e).label.toString(), new TempList(r, null), null));
        return r;
    }
    return null;
}
    ...
    Temp munchCONST(CONST e){
        Temp r = new Temp();
        TempList dstList  = new TempList(r, null);
        // CORRIGIDO: A lista de destino agora é o segundo argumento.
